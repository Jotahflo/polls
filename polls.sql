...
select content, count(content) as amount from answers where id_question = 2 group by content;
select full_name, count(full_name) as amount from users group by full_name;